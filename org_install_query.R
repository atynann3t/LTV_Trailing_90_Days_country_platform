...
FROM (
      ...
      FROM (
        ...
        FROM (
          ...
          LEFT JOIN EACH (
            ...
            FROM
            [n3twork-legendary-analytics:DAILY_GAMERIDS.gamerids_poole_summary] AS a
            LEFT JOIN (
              SELECT
              STRING(UPPER(country_code)) AS country_code,
              country
              FROM
              [n3twork-marketing-analytics:TEMP_ANALYSIS.country_code_lookup] ) b
            ON
  ...
  WHERE
  user_profile.joindate >= '2017-01-01') a
CROSS JOIN (
  SELECT
